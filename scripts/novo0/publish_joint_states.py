...
class JointStatePublisher():
    ...
    def loop(self, joint_values_from_ur5):
        # hz = get_param("rate", 10)  # 10hz
        # r = rospy.Rate(hz)
        rospy.loginfo(rospy.get_caller_id() + "I heard %s", joint_values_from_ur5)
        self.flag = 1
        delta = get_param("delta", 0.0)

        self.joint_values.position = joint_values_from_ur5.position
        ...
        for i, name in enumerate(self.joint_list):
            ...
            if has_position and 'position' in joint:
                ...
                # print("Posicao da junta 6: " + str(msg.position[6]))
                # print i
                # print(joint['position'])
                if i == 6:
                    msg.position[i] = self.joint_values.position[0] * factor + offset
                elif i == 7:
                    msg.position[i] = self.joint_values.position[1] * factor + offset
                elif i == 8:
                    msg.position[i] = self.joint_values.position[2] * factor + offset
                elif i == 9:
                    msg.position[i] = self.joint_values.position[3] * factor + offset
                elif i == 10:
                    msg.position[i] = self.joint_values.position[4] * factor + offset
                elif i == 11:
                    msg.position[i] = self.joint_values.position[5] * factor + offset
    ...
    def listener(self):
        ...
        rospy.Subscriber('joint_states_ur5', JointState, self.loop)
        hz = get_param("rate", 10)  # 10hz
        rate = rospy.Rate(hz)
