...
from potential_field_jac import PotentialField
import csv
...
def generate_path():
    ...
    path = potential_field.simulate(start_position, goal_position, obstacles, current_joint_angles)   # Usando o método simulate da classe PotentialField
    return path
...
def loop(current_joint_angles):
    global path, start_position, JA
...
if __name__ == '__main__':
    try:
        ...
        loop(current_joint_angles) #function main.
...
# # block responsable to determine the atractives forces
# def get_joint_forces(self, position, goal, q_atual):
...
# jacob = calcular_jacobiano(q_atual)
