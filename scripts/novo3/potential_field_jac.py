...
class PotentialField:
    ...
    def set_obstacles(self, obstacles_list):
        ...
        #block responsable to determine the jacobiana matrix
    def calcular_jacobiano(self, q_atual):
    ...
    def total_force(self, position, goal, q_atual):
        """Calcula a força total em 3D, considerando múltiplos obstáculos."""

        #Calcular o torque nas juntas utilizando a matriz jacobiana
        Jacobian = self.calcular_jacobiano(q_atual)  

        Jv = Jacobian[0:3,:]   # Matriz Jacobiana de translação
        Jw = Jacobian[3:6,:]   # Matriz Jacobiana de rotação
        ...
        # Força total (Torque nas juntas)
        tot_forc = np.dot(Jw.T, att_force) + np.dot(Jv.T, att_force) + np.dot(Jv.T, total_repulsive_force) 
    ...
    def convert_cart_cood(self, current_position, q_atual):
        '''
        this function is responsible for obtaining the joint angles from the cartesian coodenation 
        using the inverse kinematics
        '''
        if self.iterations == 0:
            JA = [q_atual]  #list for store the joint 
        
        #calculates the joints angles with the inverse kinematics
        point = current_position
        joint_angles = inverse_kinematics_ur5(point[0], point[1], point[2], q_atual)    
    ...
    def update_position(self, position, force):
        ...
        up_pos = np.array(position) + self.step_size * np.array(force[0:3])
        return up_pos

    def simulate(self, start_position, goal, obstacles_list, q_atual, max_iterations=100, tolerance=0.1):
        ...
        while np.linalg.norm(current_position - goal) > tolerance and self.iterations < max_iterations:
            force = self.total_force(current_position, goal, q_atual)
            current_position = self.update_position(current_position, force[0])
            path.append(current_position)
            q_atual = self.convert_cart_cood(current_position, q_atual)
            self.iterations += 1
...
# q_atual = [0, -1.57, 0, 0, 0, -1.57]
# Jacobian = cp.calcular_jacobiano(q_atual)
# print(Jacobian[0:3,:])

# determinant = np.linalg.det(Jacobian)
# print("Determinante da matriz Jacobiana:", determinant)
